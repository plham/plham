...
*/		}
		...
		|| getZ( presentMarketPricesStep(time) ) <  Matrix.minimum(translation6(presentMarketPricesStep(time),this.orderMarket)) 
			...
			x = Matrix.ActiveSetMethod(this.C,this.D,A,B,initialValue);
			/*for(i in 0..(x.size-1)){
				x(i) = -1.0*(Matrix.maxPositionSize as Double);
			}*/
			if(!checkASMError(x)){
				//Console.OUT.println("##hillclimb Start!");
				val init = translationm1(translation0(initialValue), this.orderMarket);
				x = translation0(translation7(translation2(maximizedVector(init), orderMarket) as Map[Long,Long], orderMarket));
			}
		}
		if (DEBUG == -2) {
			//Console.OUT.println("**continuousOPTIMAL");
			//Matrix.dump(x);
			//Console.OUT.println("**continuousOPTIMALCHECK:"+Matrix.checkInitialASM(C,D,x));
		}
		//assert Matrix.checkInitialASM(this.C,this.D,x): "continuousOPTIMALCHECKError";
		//x(0) = -0.5;
		//x(1) = 0.5;
		//最適ポートフォリオのベクトルX(離散値)の候補を返す。
		candidates =  FCNMarkowitzPortfolioAgent.getLongCandidates(x, this.orderMarket);
		if (DEBUG == -2) {
		...
		//候補値の中から目的関数を最大化する最適ポートフォリオのベクトルX(離散値)を見つけて返す。
		y = argMaximize(candidates);

		if (DEBUG == -2) {
			Console.OUT.println("**optimal:");
			var opt:Rail[Double] = FCNMarkowitzPortfolioAgent.translation5(y,orderMarket);
			Matrix.dump(opt);
		}
...
//				out(i)(j) = (this.b/(Z*Z))*expectedRisk.get(orderMarket(j)).get(orderMarket(i))*expectedLogReturns.get(orderMarket(i))*expectedLogReturns.get(orderMarket(j))*expectedPrices.get(orderMarket(i))*expectedPrices.get(orderMarket(j));
	...
	public static def main(args:Rail[String]) {
		...
		var s0:Rail[Double] = new Rail[Double](2);
			s0(0) = 400.0;
			s0(1) = 400.0;
		var mu:Rail[Double] = new Rail[Double](2);
			mu(0) = 0.0;
			mu(1) = 0.0;
		var sigma:Rail[Double] = new Rail[Double](2);
			sigma(0) =  0.0005; //1.0; 
			sigma(1) =  0.0005; //1.0;   
		var cor:Rail[Rail[Double]] = new Rail[Rail[Double]](2);
			cor(0) = new Rail[Double](2);
				cor(0)(0) = 1.0;
				cor(0)(1) = 0.0;
			cor(1) = new Rail[Double](2);
				cor(1)(0) = 0.0;
				cor(1)(1) = 1.0;
		val dt = 1.0;
		val logType =true;
		val seed:Long =1;
		val random = new Random(seed); // MEMO: main()

		for(var i:Long = 0; i<2; i++){		
			markets.add(markets2.get(i));
		}


		agent.accessibleMarkets = markets;
		agent.setMarketAccessible(m1.id);
		agent.setMarketAccessible(m2.id);
		agent.allMarkets = markets2;
		agent.orderMarket = agent.marketOrder();
		agent.fundamentalMeanReversionTime = 100.0;
		agent.noiseScale = 0.001;
		agent.random = random;
		agent.fundamentalWeight = 10.0;
		agent.chartWeight = 1.0;
		agent.noiseWeight = 10.0;
		agent.covarfundamentalWeight = 0.0;
		agent.setCashAmount(15000.0);
		agent.setAssetVolume(m1.id, 50);
		agent.setAssetVolume(m2.id, 50);
		agent.b = 0.001;
		agent.leverageRate = 1.0;
		agent.shortSellingAbility = false;
		//理論価格・市場価格の系列生成
		val mgbm = new MultiGeomBrownian2(random, s0, mu, sigma, cor, dt,logType);

		var resentFundamentalPrices:HashMap[Market,ArrayList[Double]] = new HashMap[Market,ArrayList[Double]]();
		var resentMarketPrices:HashMap[Market,ArrayList[Double]] = new HashMap[Market,ArrayList[Double]]();

		val size = 100; //1000;

		var priceF1:ArrayList[Double] = new ArrayList[Double](size+1);
		var priceF2:ArrayList[Double] = new ArrayList[Double](size+1);
		var priceM1:ArrayList[Double] = new ArrayList[Double](size+1);
		var priceM2:ArrayList[Double] = new ArrayList[Double](size+1);

		for (t in 0..size) {
			val X:Rail[Double] = mgbm.nextBrownian();
			var o:Double = priceF1.set(X(0), (t as Int));
			o = priceF2.set(X(1), (t as Int));
			if( t<=1 ){
				//o = priceM1.set( X(0) + random.nextDouble(), (t as Int) );
				//o = priceM2.set( X(1) + random.nextDouble(), (t as Int) );
				o = priceM1.set( X(0), (t as Int) );
				o = priceM2.set( X(1), (t as Int) );
			}else{
				val b = new Rail[Double](2);
				val r1 = random.nextDouble();
				val r2 = random.nextDouble();
				b(0) = priceM1.get(t-1)*r1 + (1.0-r1)*priceF1.get(t);
				b(1) = priceM2.get(t-1)*r2 + (1.0-r2)*priceF2.get(t);

				val d = new Rail[Double](2);
				d(0) = random.nextDouble()*( priceM1.get(t-1) -priceM1.get(t-2) );
				d(1) = random.nextDouble()*( priceM2.get(t-1) -priceM2.get(t-2) );

				//o = priceM1.set( b(0) +  b(0)*( 2.0/( 1.0 +Math.exp(-1.0*d(0)) ) -1.0 )/2 , (t as Int) );
				//o = priceM2.set( b(1) +  b(1)*( 2.0/( 1.0 +Math.exp(-1.0*d(1)) ) -1.0 )/2, (t as Int) );
		...
		resentFundamentalPrices.put(m1,priceF1);
		resentFundamentalPrices.put(m2,priceF2);
		resentMarketPrices.put(m1,priceM1);
		resentMarketPrices.put(m2,priceM2);

/*		for (t in 0..1000) {
			Console.OUT.print(resentFundamentalPrices.get(m1).get(t) + "\t");
			Console.OUT.print(resentMarketPrices.get(m1).get(t) + "\t");
			Console.OUT.print(resentFundamentalPrices.get(m2).get(t) + "\t");
			Console.OUT.print(resentMarketPrices.get(m2).get(t) + "\t");
			Console.OUT.println();
		}
		Console.OUT.println("==================");
*/

		var logReturnF1:ArrayList[Double] = new ArrayList[Double](size);
		var logReturnF2:ArrayList[Double] = new ArrayList[Double](size);
		var logReturnM1:ArrayList[Double] = new ArrayList[Double](size);
		var logReturnM2:ArrayList[Double] = new ArrayList[Double](size);

		for (t in 0..(size-1)) {
			var r:Double = logReturnF1.set(Math.log( resentFundamentalPrices.get(m1).get(t+1)/resentFundamentalPrices.get(m1).get(t) ),(t as Int) );
			r = logReturnF2.set(Math.log( resentFundamentalPrices.get(m2).get(t+1)/resentFundamentalPrices.get(m2).get(t) ), (t as Int) );
			r = logReturnM1.set(Math.log( resentMarketPrices.get(m1).get(t+1)/resentMarketPrices.get(m1).get(t) ),(t as Int));
			r = logReturnM2.set(Math.log( resentMarketPrices.get(m2).get(t+1)/resentMarketPrices.get(m2).get(t) ),(t as Int));
		}

		var recentFundamentalLogReturns:HashMap[Market,ArrayList[Double]] = new HashMap[Market,ArrayList[Double]]();
		recentFundamentalLogReturns.put(m1, logReturnF1);
		recentFundamentalLogReturns.put(m2, logReturnF2);


		var recentMarketLogReturns:HashMap[Market,ArrayList[Double]] = new HashMap[Market,ArrayList[Double]]();
		recentMarketLogReturns.put(m1, logReturnM1);
		recentMarketLogReturns.put(m2, logReturnM2);

/*		for (t in 0..999) {
			Console.OUT.print(recentFundamentalLogReturns.get(m1).get(t) + "\t");
			Console.OUT.print(recentMarketLogReturns.get(m1).get(t) + "\t");
			Console.OUT.print(recentFundamentalLogReturns.get(m2).get(t) + "\t");
			Console.OUT.println(recentMarketLogReturns.get(m2).get(t));
		}
		Console.OUT.println("==================");
*/
		var presentFundamentalPrices:HashMap[Market,Double] = new HashMap[Market,Double]();
			presentFundamentalPrices.put(m1, resentFundamentalPrices.get(m1).get(size) );
			presentFundamentalPrices.put(m2, resentFundamentalPrices.get(m2).get(size) );
		var presentMarketPrices:HashMap[Market,Double] = new HashMap[Market,Double]();
			presentMarketPrices.put(m1, resentMarketPrices.get(m1).get(size) );
			presentMarketPrices.put(m2, resentMarketPrices.get(m2).get(size) );


		//リスクを求める.
		val expectedRisk = agent.expectedRisk(recentMarketLogReturns,recentFundamentalLogReturns,agent.covarfundamentalWeight);
		agent.expectedRisk = FCNMarkowitzPortfolioAgent.translation10(expectedRisk, agent.orderMarket);
		//Matrix.dump(FCNMarkowitzPortfolioAgent.translation12(expectedRisk,agent.orderMarket));

		//fundamentalist,chartist,noiseの手法それぞれで期待リターンを求め,それらを合わせて，期待リターン,期待価格を求める.
		val recentFundamentalistReturns:HashMap[Market,Double] = agent.recentFundamentalistLogReturns(presentFundamentalPrices, presentMarketPrices);
		val recentChartistReturns:HashMap[Market,Double] = agent.recentChartistReturns(recentMarketLogReturns);
		val noiseReturns:HashMap[Market,Double] = agent.recentNoiseReturns();
		val expectedFCNReturns = agent.expectedFCNReturns(recentFundamentalistReturns, recentChartistReturns,noiseReturns);
		agent.expectedFCNReturns = FCNMarkowitzPortfolioAgent.translation1(expectedFCNReturns,agent.orderMarket);
		var expectedFCNPrices:HashMap[Market,Double] = agent.expectedLogPrices(presentMarketPrices,expectedFCNReturns);

		agent.expectedPrices = FCNMarkowitzPortfolioAgent.translation1(expectedFCNPrices,agent.orderMarket); 



		//最適ポートフォリオのベクトルXの計算に必要なAX = B の行列AとベクトルBを計算。
		//(総資産Zは今期tの市場価格を使う。一方、目的関数内の価格については予想値を使う)
		agent.Z = agent.getZ(presentMarketPrices);
		var B:Rail[Double] = FCNMarkowitzPortfolioAgent.getB(expectedFCNPrices, expectedFCNReturns, agent.Z, agent.orderMarket);
		var A:Rail[Rail[Double]] = FCNMarkowitzPortfolioAgent.getA(expectedFCNPrices, expectedFCNReturns, expectedRisk, agent.Z, agent.b, agent.orderMarket);
		//var A:Rail[Rail[Double]] = FCNMarkowitzPortfolioAgent.translation12(expectedRisk,agent.orderMarket);
/*
		Console.OUT.print("\n"+"\t*Q=");
		Matrix.dump(A);
		Console.OUT.print("\t*C=");
...
*/
		...
		val multiplier:Double = Matrix.multiplier(Math.min(agent.sizeMinA(A),agent.sizeMinB(B)));
		//Aの各セル，Bの各要素となる値の中で最も絶対値が小さな値の絶対値を取ってきて，その値の逆数と同じ桁数の数字をmulitplierとする.
		...
		var rankE:Long = Matrix.rank( Matrix.echelonForm(Matrix.ExpansionCoefficientMatrix(A,B)) );
		//バーゼル以外の必要な制約条件（線形式）を考慮に入れて再計算（値が非負など）
		var constraints:Rail[Rail[Double]] = FCNMarkowitzPortfolioAgent.constraints(A.size,agent.shortSellingAbility,agent.Z,agent.maxPositionSize,agent.leverageRate,presentMarketPrices,agent.orderMarket);
		//Console.OUT.println("**const");
		//Matrix.dump(constraints);
		agent.C = Matrix.devideExpansionCoefficientMatrix1(constraints);
		agent.D = Matrix.devideExpansionCoefficientMatrix2(constraints);
		...
		//原点を代入し，原点が制約条件を全て満たすことを確認.
		var initialValue:Rail[Double] =  Matrix.initialASM0(translation6(presentMarketPrices,agent.orderMarket),agent.Z,0.9);
		...
		x = Matrix.ActiveSetMethod(agent.C,agent.D,A,B,initialValue);

		Console.OUT.println("**continuousOPTIMAL");
		Matrix.dump(x);
		//Matrix.dump(x2);
		Console.OUT.println("**continuousOPTIMALCHECK:"+Matrix.checkInitialASM(agent.C,agent.D,x));
		var max:Double = agent.targetFunctionD(agent.translationm2(x,agent.orderMarket));

		Console.OUT.println("**optValue:"+max +"\n" );

		//最適ポートフォリオのベクトルX(離散値)の候補を返す。
		candidates =  FCNMarkowitzPortfolioAgent.getLongCandidates(x, agent.orderMarket);

/*		Console.OUT.println("*candidates1:");
		for(var i:Long = 0; i<candidates.size(); i++){
			Console.OUT.println("**hoge"+i);
			Matrix.dump(FCNMarkowitzPortfolioAgent.translation5(candidates.get(i),agent.orderMarket));
			max = agent.targetFunction(candidates.get(i));
			Console.OUT.println("***value:"+max +"\n" );
		}
*/

		//制約条件で更に候補者を絞る.
		candidates =  FCNMarkowitzPortfolioAgent.getLongConstrainedCandidates(candidates,agent.C,agent.D, agent.orderMarket);
/*		Console.OUT.println("*candidates2:");
		for(var i:Long = 0; i<candidates.size(); i++){
			Console.OUT.println("**geho"+i);
			Matrix.dump(FCNMarkowitzPortfolioAgent.translation5(candidates.get(i),agent.orderMarket));
...
*/
		//候補値の中から目的関数を最大化する最適ポートフォリオのベクトルX(離散値)を見つけて返す。
		y = agent.argMaximize(candidates);

		Console.OUT.println("**optimal:");
		var opt:Rail[Double] = FCNMarkowitzPortfolioAgent.translation5(y,agent.orderMarket);
		Matrix.dump(opt);
		max = agent.targetFunction(y);
		Console.OUT.println("***value:"+max+"\n" );

		assert Matrix.checkInitialASM(agent.C,agent.D,FCNMarkowitzPortfolioAgent.translation5(y,agent.orderMarket)):"optimalCheckError";

 		val init = FCNMarkowitzPortfolioAgent.translation3(agent.assetsVolumes, agent.orderMarket);
		val x2 = FCNMarkowitzPortfolioAgent.translation0(FCNMarkowitzPortfolioAgent.translation7(FCNMarkowitzPortfolioAgent.translation2(agent.maximizedVector(init), agent.orderMarket) as Map[Long,Long], agent.orderMarket));

		Console.OUT.println("**continuousOPTIMAL2");
		Matrix.dump(x2);

	}

	public def cancelLastMarketOrders(){
		var m:Long = 0;
		var n:Long = this.lastOreders.size();
		for (o in this.lastOreders) { 
			if(o.kind == Order.KIND_BUY_MARKET_ORDER || o.kind == Order.KIND_SELL_MARKET_ORDER ){
				this.lastOreders.remove(o);
			}
			m++;
		}
		assert m == n: "cancelLastMarketOrders";
	}
	public static def register(sim:Simulator):void {
		...
		sim.addAgentInitializer(className, (range:LongRange, json:JSON.Value, container:Settable[Long, Agent]) => {
			for (i in range) {
				container(i) = createFCNMarkowitzPortfolioAgent(json, sim);
			}
		});
	}

	public static def createFCNMarkowitzPortfolioAgent(json:JSON.Value, sim:Simulator):FCNMarkowitzPortfolioAgent {
		val jsonrandom = new JSONRandom(sim.getRandom());
		return new FCNMarkowitzPortfolioAgent().setupFCNMarkowitzPortfolioAgent(json, jsonrandom, sim);
	}

	public def setupFCNMarkowitzPortfolioAgent(json:JSON.Value, random:JSONRandom, sim:Simulator):FCNMarkowitzPortfolioAgent {
